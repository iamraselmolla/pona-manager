...
export const DailyReportScreen = () => {
  ...
  useEffect(() => {
    reportAPI.getDaily(date).then((res) => {
  ...
  const { totalOrders = 0, totalSales = 0, totalExpenses = 0, totalProfit = 0, totalDue = 0, totalCollections = 0 } = report;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Row label="Total Sales" value={formatCurrency(totalSales)} highlight={false} />
        <Row label="Collections" value={formatCurrency(totalCollections)} highlight={false} />
        <Row label="Total Due" value={formatCurrency(totalDue)} highlight={false} />
        <Row label="Expenses" value={formatCurrency(totalExpenses)} highlight={false} />
        <Row label="Profit/Loss" value={formatCurrency(totalProfit)} highlight={true} />
      </View>
...
const styles = StyleSheet.create({
  ...
  rowHighlight: { backgroundColor: COLORS.successLight, borderRadius: 6, paddingHorizontal: 8, borderBottomWidth: 0 },
