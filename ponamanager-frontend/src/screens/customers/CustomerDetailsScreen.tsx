  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.statsRow}>
        <StatBox label="Orders" value={customer.totalOrders.toString()} color={COLORS.primary} />
        <StatBox label="PL Bought" value={customer.totalPLPurchased.toLocaleString()} color={COLORS.info} />
        <StatBox label="Paid" value={formatCurrency(customer.totalPaid)} color={COLORS.success} />
        <StatBox label="Due" value={formatCurrency(customer.totalDue)} color={COLORS.danger} />
      </View>
              ...
              return (
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.orderQty}>{order.plQuantity.toLocaleString()} PL</Text>
                  ...
                  <View style={{ alignItems: 'flex-end', gap: 4 }}>
                    ...
                    {order.dueAmount > 0 && (
                      <Text style={styles.dueText}>Due: {formatCurrency(order.dueAmount)}</Text>
                    )}
                  </View>
                </TouchableOpacity>
              );
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(255,255,255,0.3)', alignItems: 'center', justifyContent: 'center' },
  ...
  address: { fontSize: 13, color: 'rgba(255,255,255,0.7)', textAlign: 'center' },
  area: { fontSize: 12, color: 'rgba(255,255,255,0.6)' },
  editBtn: { flexDirection: 'row', gap: 4, alignItems: 'center', backgroundColor: COLORS.white, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, marginTop: 6 },
  editBtnText: { color: COLORS.primary, fontWeight: '700', fontSize: 13 },
  statsRow: { flexDirection: 'row', padding: 12, gap: 8 },
  statBox: { flex: 1, backgroundColor: COLORS.white, borderRadius: 10, padding: 12, alignItems: 'center', borderTopWidth: 3, elevation: 1 },
  ...
  primaryBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: COLORS.primary, borderRadius: 10, padding: 12, justifyContent: 'center' },
  primaryBtnText: { color: COLORS.white, fontWeight: '700', fontSize: 14 },
  tabs: { flexDirection: 'row', marginHorizontal: 12, backgroundColor: COLORS.white, borderRadius: 10, padding: 4, marginBottom: 8 },
  ...
  orderCard: { flexDirection: 'row', backgroundColor: COLORS.white, borderRadius: 10, padding: 14, elevation: 1 },
  ...
  paymentCard: { flexDirection: 'row', gap: 12, alignItems: 'center', backgroundColor: COLORS.white, borderRadius: 10, padding: 14, elevation: 1 },
  paymentAmount: { fontSize: 16, fontWeight: '800', color: COLORS.success },
  paymentDate: { fontSize: 12, color: COLORS.textMuted },
  paymentNote: { fontSize: 12, color: COLORS.textSecondary, fontStyle: 'italic' },
