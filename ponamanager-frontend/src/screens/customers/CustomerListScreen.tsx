...
const CustomerItem = ({ customer, onPress }: { customer: Customer; onPress: () => void }) => (
  <TouchableOpacity style={styles.customerCard} onPress={onPress} activeOpacity={0.8}>
...
export const CustomerListScreen = () => {
  ...
  const fetchCustomers = useCallback(async (reset = false) => {
    try {
      ...
      const res = await customerAPI.getAll({ search, page: currentPage, limit: 20 });
  ...
  useEffect(() => { fetchCustomers(true); }, [search]);
...
const styles = StyleSheet.create({
  ...
  badge: { backgroundColor: COLORS.successLight, borderRadius: 4, paddingHorizontal: 6, paddingVertical: 1 },
  ...
  dueAmount: { fontSize: 13, fontWeight: '700', color: COLORS.danger, marginBottom: 2 },
