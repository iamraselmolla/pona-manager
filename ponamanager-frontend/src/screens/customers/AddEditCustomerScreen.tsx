...
export const AddEditCustomerScreen = () => {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      }).catch(() => { Alert.alert('Error', 'Failed to load customer'); navigation.goBack(); });
    }
  }, []);

  const validate = () => {
    if (!name.trim()) { Alert.alert('Validation', 'Name is required'); return false; }
    if (!mobile.trim() || !/^01[3-9]\d{8}$/.test(mobile)) { Alert.alert('Validation', 'Enter valid mobile (e.g. 01XXXXXXXXX)'); return false; }
    return true;
  };
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const payload = { name: name.trim(), mobile: mobile.trim(), address: address.trim(), area: area.trim() };
      if (isEdit) {
  ...
  if (fetchLoading) return <View style={styles.center}><ActivityIndicator size="large" color={COLORS.primary} /></View>;

  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        <View style={styles.card}>
          <Field label="Full Name" value={name} onChangeText={setName} placeholder="Customer full name" required />
          <Field label="Mobile Number" value={mobile} onChangeText={setMobile} placeholder="01XXXXXXXXX" keyboardType="phone-pad" required />
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.white, borderRadius: 12, padding: 16, gap: 4, marginBottom: 16 },
