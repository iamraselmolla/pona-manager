...
export const AddExpenseScreen = () => {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      expenseAPI.getById(expenseId).then((res) => {
      ...
      }).catch(() => { Alert.alert('Error', 'Failed to load expense'); navigation.goBack(); });
    }
  }, []);

  const handleSave = async () => {
    if (!amount) { Alert.alert('Validation', 'Amount is required'); return; }
    setLoading(true);
    try {
      const payload = { amount: parseFloat(amount), category, date, notes };
      if (isEdit) {
        await expenseAPI.update(expenseId, payload);
        Alert.alert('Success', 'Expense updated', [{ text: 'OK', onPress: () => navigation.goBack() }]);
      } else {
...
const styles = StyleSheet.create({
  ...
  categoryBtn: { flex: 1, minWidth: '45%', padding: 8, borderRadius: 8, borderWidth: 1.5, borderColor: COLORS.border, alignItems: 'center' },
