...
export const ExpenseListScreen = () => {
  ...
  const fetchExpenses = useCallback(async () => {
    try {
      const res = await expenseAPI.getAll({ category: categoryFilter || undefined });
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.filterBtn, categoryFilter === item && styles.filterBtnActive]}
              ...
              <View style={styles.expenseCard}>
                ...
                <View style={{ alignItems: 'flex-end', gap: 6 }}>
                  ...
                  <View style={styles.actionBtns}>
                    <TouchableOpacity onPress={() => navigation.navigate('AddExpense', { expenseId: item.id })}>
...
const styles = StyleSheet.create({
  ...
  totalCard: {
    backgroundColor: COLORS.primary, margin: 12, borderRadius: 12,
    padding: 16, alignItems: 'center',
  },
  totalLabel: { fontSize: 13, color: 'rgba(255,255,255,0.8)' },
  totalValue: { fontSize: 28, fontWeight: '900', color: COLORS.white },
  filterBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.border },
  filterBtnActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  filterText: { fontSize: 12, fontWeight: '600', color: COLORS.textSecondary },
  filterTextActive: { color: COLORS.white },
  expenseCard: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    backgroundColor: COLORS.white, borderRadius: 12, padding: 14, elevation: 1,
  },
