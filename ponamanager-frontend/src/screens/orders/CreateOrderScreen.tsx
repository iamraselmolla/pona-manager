...
const InputField = ({ label, value, onChangeText, placeholder, keyboardType, required, editable = true }: any) => (
...
export const CreateOrderScreen = () => {
    ...
    try {
      ...
      if (c.hasRunningOrder) {
        Alert.alert('Warning', `${c.name} has a running order. Do you want to continue?`, [
          { text: 'Cancel', style: 'cancel', onPress: () => { setMobile(''); setCustomer(null); setName(''); setAddress(''); } },
  ...
  const handleMobileChange = (val: string) => {
    ...
    if (val.length < 11) { setCustomer(null); setName(''); setAddress(''); }
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.field}>
            ...
            <View style={styles.mobileRow}>
              <TextInput
                ...
                maxLength={11}
              />
              {customerLoading && <ActivityIndicator color={COLORS.primary} style={{ position: 'absolute', right: 12 }} />}
          ...
          {customer && (
            <TouchableOpacity style={styles.customerInfo} onPress={() => setShowHistory(true)}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.customerInfoSub}>Orders: {customer.totalOrders} | Due: {formatCurrency(customer.totalDue)}</Text>
              </View>
              <Text style={styles.viewHistory}>History →</Text>
            </TouchableOpacity>
          )}

          <InputField label="Customer Name" value={name} onChangeText={setName} placeholder="Full name" required editable={!customer} />
          <InputField label="Address" value={address} onChangeText={setAddress} placeholder="Address" editable={!customer} />
        </View>
        ...
        <View style={styles.card}>
          ...
          {/* Pona Type */}
          <View style={styles.field}>
            <Text style={styles.label}>Pona Type <Text style={{ color: COLORS.danger }}>*</Text></Text>
            <View style={styles.typeRow}>
              {PONA_TYPES.map((t) => (
                ...
                >
                  <Text style={[styles.typeBtnText, ponaType === t && styles.typeBtnTextActive]}>{t}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>

          <InputField label="PL Quantity" value={plQuantity} onChangeText={setPlQuantity} placeholder="0" keyboardType="numeric" required />
          <InputField label="Unit Rate (৳)" value={unitRate} onChangeText={setUnitRate} placeholder="0.00" keyboardType="numeric" required />
          ...
          <InputField label="Advance Amount (৳)" value={advanceAmount} onChangeText={setAdvanceAmount} placeholder="0" keyboardType="numeric" />

          <View style={[styles.calcRow, { backgroundColor: COLORS.dangerLight, borderRadius: 8, padding: 10 }]}>
            <Text style={[styles.calcLabel, { color: COLORS.danger }]}>Due Amount</Text>
            <Text style={[styles.calcValue, { color: COLORS.danger }]}>{formatCurrency(dueAmount)}</Text>
          </View>

          <View style={styles.field}>
            <Text style={styles.label}>Delivery Date <Text style={{ color: COLORS.danger }}>*</Text></Text>
            <TextInput
              style={styles.input}
              value={deliveryDate}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.white, borderRadius: 12, padding: 16, marginBottom: 16, gap: 4 },
  ...
  inputDisabled: { backgroundColor: COLORS.border + '40', color: COLORS.textSecondary },
  ...
  customerInfo: {
    ...
    backgroundColor: COLORS.successLight, borderRadius: 8, padding: 10, marginBottom: 14,
  },
  ...
  typeBtn: { flex: 1, padding: 8, borderRadius: 8, borderWidth: 1.5, borderColor: COLORS.border, alignItems: 'center' },
  typeBtnActive: { borderColor: COLORS.primary, backgroundColor: COLORS.successLight },
  typeBtnText: { fontSize: 11, fontWeight: '600', color: COLORS.textSecondary },
  typeBtnTextActive: { color: COLORS.primary },
  calcRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 },
