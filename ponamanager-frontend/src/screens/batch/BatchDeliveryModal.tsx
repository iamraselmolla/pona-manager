...
const FieldRow = ({ label, value, onChangeText, keyboardType, placeholder, editable = true, suffix }: any) => (
...
export const BatchDeliveryModal = () => {
  ...
  const validate = () => {
    if (!deliveredQty || delivered <= 0) { Alert.alert('সতর্কতা', 'ডেলিভারি পরিমাণ দিন'); return false; }
    if (!deliveryRate || rate <= 0)      { Alert.alert('সতর্কতা', 'দর (রেট) দিন');       return false; }
    return true;
  };
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.customerCard, { borderTopColor: typeColor }]}>
          ...
          <View style={{ alignItems: 'flex-end', gap: 4 }}>
            ...
            {advance > 0 && (
              <Text style={styles.advanceText}>জমা: {formatCurrency(advance)}</Text>
            )}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  customerCard: {
    ...
    borderTopWidth: 4, marginBottom: 14, elevation: 2,
  },
  ...
  cardTitle: { fontSize: 13, fontWeight: '800', color: COLORS.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 },
  ...
  fieldInputWrapper: { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: COLORS.border, borderRadius: 8, backgroundColor: COLORS.background, flex: 1 },
  fieldInput: { flex: 1, paddingVertical: 8, paddingHorizontal: 10, fontSize: 15, color: COLORS.text },
  ...
  calcRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 7, borderBottomWidth: 1, borderBottomColor: COLORS.border + '50' },
  ...
  dueDateWrapper: {
    ...
    backgroundColor: COLORS.dangerLight, borderRadius: 8, padding: 10,
  },
  dueDateLabel: { fontSize: 12, color: COLORS.danger, fontWeight: '600' },
  dueDateInput: { flex: 1, fontSize: 14, color: COLORS.danger, fontWeight: '700', borderWidth: 1, borderColor: COLORS.danger, borderRadius: 6, padding: 6 },

  notesInput: {
    borderWidth: 1.5, borderColor: COLORS.border, borderRadius: 8,
    padding: 10, fontSize: 14, color: COLORS.text, minHeight: 70, textAlignVertical: 'top',
  },
