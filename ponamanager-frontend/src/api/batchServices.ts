...
export const batchAPI = {
  ...
  addOrders: (batchId: string, orderIds: string[]) =>
    apiClient.patch<ApiResponse<Batch>>(`/batches/${batchId}/add-orders`, { orderIds }),

  // Remove order from pending batch
  removeOrder: (batchId: string, batchOrderId: string) =>
    apiClient.delete<ApiResponse<null>>(`/batches/${batchId}/orders/${batchOrderId}`),

  // Record delivery for a single batchOrder (opens modal)
  recordDelivery: (batchId: string, batchOrderId: string, data: {
  ...
  completeBatch: (batchId: string, expenses: { label: string; amount: number }[]) =>
    apiClient.patch<ApiResponse<Batch>>(`/batches/${batchId}/complete`, { expenses }),
