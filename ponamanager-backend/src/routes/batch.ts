...
async function recomputeBatch(batchId: string) {
  ...
  let totalOrderedGolda = 0, totalDeliveredGolda = 0;
  let totalOrderedBagda = 0, totalDeliveredBagda = 0;
  let totalOrderedVannamei = 0, totalDeliveredVannamei = 0;
  let totalCollected = 0, totalDue = 0;
  let duePendingCount = 0, pendingDeliveries = 0;

  for (const bo of batchOrders) {
    const qty = bo.order.plQuantity;
    const pona = bo.order.ponaType;

    if (pona === 'Golda PL')    { totalOrderedGolda    += qty; if (bo.deliveryStatus === 'delivered') totalDeliveredGolda    += bo.deliveredQuantity || 0; }
    if (pona === 'Bagda PL')    { totalOrderedBagda    += qty; if (bo.deliveryStatus === 'delivered') totalDeliveredBagda    += bo.deliveredQuantity || 0; }
    if (pona === 'Vannamei PL') { totalOrderedVannamei += qty; if (bo.deliveryStatus === 'delivered') totalDeliveredVannamei += bo.deliveredQuantity || 0; }

    if (bo.deliveryStatus === 'pending') { pendingDeliveries++; }
    else {
  ...
  return prisma.batch.update({
    where: { id: batchId },
    data: {
      totalOrderedGolda, totalDeliveredGolda,
      totalOrderedBagda, totalDeliveredBagda,
      totalOrderedVannamei, totalDeliveredVannamei,
      totalCollected, totalDue, totalExpenses, totalProfit,
      pendingDeliveries, duePendingCount, status,
    },
...
router.get('/monthly/:month', async (req, res) => {
  try {
    ...
    const summary = {
      ...
      completedBatches: batches.filter(b => b.status === 'completed' || b.status === 'has_due').length,
...
router.post('/', async (req, res) => {
  try {
    const { batchDate, orderIds } = req.body as { batchDate: string; orderIds: string[] };
    if (!orderIds?.length) return res.status(400).json({ success: false, message: 'No orders selected' });
    ...
    const batch = await prisma.$transaction(async (tx) => {
      const newBatch = await tx.batch.create({
        data: { batchNumber, batchDate, status: 'pending', pendingDeliveries: orderIds.length },
      });

      // Create batch orders
      await tx.batchOrder.createMany({
        data: orderIds.map((orderId) => ({ batchId: newBatch.id, orderId, deliveryStatus: 'pending' })),
      });
...
router.patch('/:id/add-orders', async (req, res) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.batchOrder.createMany({
        data: orderIds.map((orderId) => ({ batchId: req.params.id, orderId, deliveryStatus: 'pending' })),
      });
      await tx.order.updateMany({
        where: { id: { in: orderIds } },
  ...
  } catch { res.status(500).json({ success: false, message: 'Failed to add orders' }); }
});

// ── DELETE /batches/:id/orders/:batchOrderId ─────────────
router.delete('/:id/orders/:batchOrderId', async (req, res) => {
  try {
    const bo = await prisma.batchOrder.findUnique({ where: { id: req.params.batchOrderId } });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.order.update({ where: { id: bo.orderId }, data: { status: 'pending', batchId: null } });
    });
    await recomputeBatch(req.params.id);
    res.json({ success: true });
  } catch { res.status(500).json({ success: false, message: 'Failed to remove order' }); }
});
...
router.patch('/:id/orders/:batchOrderId/deliver', async (req, res) => {
  try {
    const {
      deliveredQuantity, deliveryRate, customerPayment,
      dueAmount, duePaymentDate, notes,
    } = req.body;
...
router.patch('/:id/complete', async (req, res) => {
  try {
    const { expenses } = req.body as { expenses: { label: string; amount: number }[] };
    ...
    if (pending > 0)
      return res.status(400).json({ success: false, message: `${pending} deliveries still pending` });

    await prisma.$transaction(async (tx) => {
      // Save expenses
      if (expenses?.length) {
        await tx.batchExpense.createMany({
          data: expenses.map(e => ({ batchId: req.params.id, label: e.label, amount: e.amount })),
        });
      }
    });
