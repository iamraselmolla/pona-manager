  ...
  try {
    ...
    const closing = await prisma.dailyClosing.create({
      data: {
        ...
        totalCompanyMir: Number(totalCompanyMir) || 0,
        totalCountingMir: Number(totalCountingMir) || 0,
        transportExpense: Number(transportExpense) || 0,
        laborExpense: Number(laborExpense) || 0,
        otherExpenses: Number(otherExpenses) || 0,
        netProfitLoss: Number(netProfitLoss) || 0,
    ...
    if (err.code === 'P2002')
      return res.status(400).json({ success: false, message: 'Already closed today' });
...
router4.get('/stats', async (req, res) => {
  try {
    ...
    const totalPLDelivered = batches.reduce(
      (s, b) => s + b.totalDeliveredGolda + b.totalDeliveredBagda + b.totalDeliveredVannamei,
      0,
    );
  ...
  try {
    ...
    const [orders, batchOrders, expenses] = await Promise.all([
      ...
      prisma.batchOrder.findMany({
        where: { deliveryStatus: 'delivered', batch: { batchDate: date } },
        include: { order: true },
      }),
      prisma.expense.findMany({ where: { date } }),
    ]);
    const totalSales = batchOrders.reduce((s, bo) => s + (bo.finalAmount || 0), 0);
    const totalCollected = batchOrders.reduce(
      (s, bo) => s + (bo.order.advanceAmount || 0) + (bo.customerPayment || 0),
      0,
    );
    ...
    res.json({
      ...
      data: {
        totalOrders: orders.length,
        totalDeliveries: batchOrders.length,
        totalSales,
        totalCollected,
        totalDue,
        totalExpenses,
...
router5.get('/monthly', async (req, res) => {
  try {
    ...
    const data = {
      ...
      totalGoldaSales: batches.reduce(
        (s, b) =>
          s +
          b.batchOrders
            .filter((bo) => bo.order.ponaType === 'Golda PL')
            .reduce((ss, bo) => ss + (bo.finalAmount || 0), 0),
        0,
      ),
      totalBagdaPL: batches.reduce((s, b) => s + b.totalDeliveredBagda, 0),
      totalBagdaSales: batches.reduce(
        (s, b) =>
          s +
          b.batchOrders
            .filter((bo) => bo.order.ponaType === 'Bagda PL')
            .reduce((ss, bo) => ss + (bo.finalAmount || 0), 0),
        0,
      ),
      totalVannameiPL: batches.reduce((s, b) => s + b.totalDeliveredVannamei, 0),
      totalVannameiSales: batches.reduce(
        (s, b) =>
          s +
          b.batchOrders
            .filter((bo) => bo.order.ponaType === 'Vannamei PL')
            .reduce((ss, bo) => ss + (bo.finalAmount || 0), 0),
        0,
      ),
...
router5.get('/expense', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { total: expenses.reduce((s, e) => s + e.amount, 0), categories },
    });
  } catch {
    res.status(500).json({ success: false, message: 'Failed' });
  }
...
router5.get('/profit-loss', async (req, res) => {
  try {
    ...
    const totalSales = batches.reduce((s, b) => s + b.totalCollected, 0);
    const totalExpenses =
      expenses.reduce((s, e) => s + e.amount, 0) + batches.reduce((s, b) => s + b.totalExpenses, 0);
    res.json({
      success: true,
      data: { totalSales, totalExpenses, netProfit: totalSales - totalExpenses },
    });
  } catch {
    res.status(500).json({ success: false, message: 'Failed' });
  }
