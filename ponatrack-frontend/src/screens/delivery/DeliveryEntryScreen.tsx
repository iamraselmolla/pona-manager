...
const CalcRow = ({ label, value, highlight }: any) => (
  <View style={[styles.calcRow, highlight && styles.calcRowHighlight]}>
    ...
    <Text style={[styles.calcValue, highlight && styles.calcValueHighlight]}>
      {value}
    </Text>
  </View>
);
...
export const DeliveryEntryScreen = () => {
  ...
  const remainingDue = Math.max(
    0,
    finalAmount - (order?.advanceAmount || 0) - payment,
  );
  const mirPercentage = order?.plQuantity
    ? ((order.plQuantity - delivered) / order.plQuantity) * 100
    : 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        err.response?.data?.message || "Failed to record delivery",
      );
    } finally {
      setSaving(false);
    }
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.orderInfo}>
          ...
          <Text style={styles.orderDetail}>
            {order?.ponaType} | Ordered: {order?.plQuantity.toLocaleString()} PL
          </Text>
          <Text style={styles.orderDetail}>
            Order Amount: {formatCurrency(order?.totalPrice || 0)}
          </Text>
          {(order?.advanceAmount || 0) > 0 && (
            <Text style={styles.advanceText}>
              Advance Paid: {formatCurrency(order?.advanceAmount || 0)}
            </Text>
          )}
        </View>
        ...
        <View style={styles.card}>
          ...
          {[
            {
              label: "Delivered Quantity (PL) *",
              val: deliveredQty,
              set: setDeliveredQty,
            },
            {
              label: "Company Provided Quantity",
              val: companyProvidedQty,
              set: setCompanyProvidedQty,
            },
          ...
          <View style={styles.mirRow}>
            <View style={styles.mirBox}>
              ...
              <Text
                style={[
                  styles.mirValue,
                  { color: companyMir >= 0 ? COLORS.danger : COLORS.success },
                ]}
              >
            ...
            <View style={styles.mirBox}>
              ...
              <Text
                style={[
                  styles.mirValue,
                  { color: countingMir >= 0 ? COLORS.danger : COLORS.success },
                ]}
              >
            ...
            <View style={styles.mirBox}>
              ...
              <Text
                style={[
                  styles.mirValue,
                  { color: mirPercentage > 5 ? COLORS.danger : COLORS.success },
                ]}
              >
        ...
        <View style={styles.card}>
          ...
          <CalcRow
            label="Advance Paid"
            value={formatCurrency(order?.advanceAmount || 0)}
          />
