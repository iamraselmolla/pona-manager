...
export const MonthlyReportScreen = () => {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>
          পোনা প্রকার বিশ্লেষণ (Pona Type Analysis)
        </Text>

        {/* Golda PL */}
        <View style={styles.ponaCard}>
          <View style={[styles.ponaHeader, { backgroundColor: "#F5A62320" }]}>
            <Text style={[styles.ponaTitle, { color: "#F5A623" }]}>
              গলদা পোনা (Golda PL)
            </Text>
          ...
          <MetricBox
            label="মোট পিএল"
            value={totalGoldaPL.toLocaleString()}
            color="#F5A623"
          />
          <MetricBox
            label="বিক্রয়"
            value={formatCurrency(totalGoldaSales)}
            color="#F5A623"
          />
          <MetricBox
            label="লাভ"
            value={formatCurrency(totalGoldaProfit)}
            color={totalGoldaProfit >= 0 ? COLORS.success : COLORS.danger}
          />
        </View>
        ...
        <View style={styles.ponaCard}>
          <View style={[styles.ponaHeader, { backgroundColor: "#1E88E520" }]}>
            <Text style={[styles.ponaTitle, { color: "#1E88E5" }]}>
              বাগদা পোনা (Bagda PL)
            </Text>
            <Text style={[styles.ponaSubtitle, { color: "#1E88E5" }]}>
              জোছনা
            </Text>
          </View>
          <MetricBox
            label="মোট পিএল"
            value={totalBagdaPL.toLocaleString()}
            color="#1E88E5"
          />
          <MetricBox
            label="বিক্রয়"
            value={formatCurrency(totalBagdaSales)}
            color="#1E88E5"
          />
          <MetricBox
            label="লাভ"
            value={formatCurrency(totalBagdaProfit)}
            color={totalBagdaProfit >= 0 ? COLORS.success : COLORS.danger}
          />
        </View>
        ...
        <View style={styles.ponaCard}>
          <View style={[styles.ponaHeader, { backgroundColor: "#43A04720" }]}>
            <Text style={[styles.ponaTitle, { color: "#43A047" }]}>
              ভানামেই পোনা (Vannamei PL)
            </Text>
            <Text style={[styles.ponaSubtitle, { color: "#43A047" }]}>
              সাদা পা
            </Text>
          </View>
          <MetricBox
            label="মোট পিএল"
            value={totalVannameiPL.toLocaleString()}
            color="#43A047"
          />
          <MetricBox
            label="বিক্রয়"
            value={formatCurrency(totalVannameiSales)}
            color="#43A047"
          />
          <MetricBox
            label="লাভ"
            value={formatCurrency(totalVannameiProfit)}
            color={totalVannameiProfit >= 0 ? COLORS.success : COLORS.danger}
          />
        </View>
      </View>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>
          অপারেশনাল মেট্রিক্স (Operational Metrics)
        </Text>
        <View style={styles.metricsRow}>
          <MetricBox
            label="কোম্পানি মীর"
      ...
      <View style={styles.actionRow}>
        <TouchableOpacity
          ...
          onPress={() => Share.share({ message: "Monthly Report" })}
        >
          <Ionicons
